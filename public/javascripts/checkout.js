...
var chg_input_element = function (parent, html) {
  ...
  parent.append(html);
  child = parent.find(':only-child');		// chaining isn't working?
  child.addClass('required')
       .attr('name', name)
       .attr('id', id)
       .val();  	// old value (eg state id) no longer valid so do not keep it
  return child;
};
